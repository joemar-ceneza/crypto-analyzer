...
# FINDINGS
# ======================================================
def _spread_finding(table: pd.DataFrame, dimension: str) -> str | None:
    """
    States the gap between the best and worst adequately-sampled group.
    ...
    ranking of one.
    """
    usable = table[table["enough"] & table["hit_rate_pct"].notna()]
    if len(usable) < 2:
        return None
    ...
    if gap < config.BREAKDOWN_MIN_GAP_PCT:
        return (
            f"{label}: no meaningful separation — best and worst are within "
            f"{gap:.0f} percentage points of each other."
        )
    return (
        f"{label}: '{best[label]}' hit {best['hit_rate_pct']:.0f}% over "
...
def _findings(tables: dict[str, pd.DataFrame], sample: dict) -> list[str]:
    ...
    When the sample cannot support conclusions the findings are withheld
    entirely rather than hedged — a caveated ranking still gets read as a
    ranking.
    """
    if not sample["sufficient"]:
        return []
    found = [_spread_finding(table, dim) for dim, table in tables.items() if not table.empty]
